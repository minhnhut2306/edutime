...
export const reportsAPI = {
  getTeacherReport: async (teacherId, type, filters = {}) => {
    ...
    return await apiRequest(
      `reports/teacher/${teacherId}?${params}`,
      "GET",
      {},
      token
    );
  ...
  exportReport: async (options) => {
    ...
    if (!schoolYearId && !schoolYear)
      throw new Error("schoolYearId hoặc schoolYear là bắt buộc");
    if (!teacherIds && !teacherId)
      throw new Error("teacherIds hoặc teacherId là bắt buộc");
    ...
    if (weekIds && weekIds.length > 0)
      params.append("weekIds", JSON.stringify(weekIds));
  ...
  exportMultipleReports: async (options) => {
    const token = localStorage.getItem("token");
    const {
      teacherIds,
      schoolYear,
    ...
    if (!teacherIds || teacherIds.length === 0)
      throw new Error("teacherIds là bắt buộc");
    ...
    const url = `reports/export/multiple?${params.toString()}`;
